...
const baseLegacyDaoAddress: object = { id: 0, legacyAddress: "", address: "" };

export const LegacyDaoAddress = {
  encode(message: LegacyDaoAddress, writer: Writer = Writer.create()): Writer {
    if (message.id !== 0) {
  ...
  decode(input: Reader | Uint8Array, length?: number): LegacyDaoAddress {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseLegacyDaoAddress } as LegacyDaoAddress;
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
