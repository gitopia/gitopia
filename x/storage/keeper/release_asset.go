...
func (k Keeper) AppendReleaseAsset(
	...
	releaseAssetKey := fmt.Sprintf("%d-%s-%s", releaseAsset.RepositoryId, releaseAsset.Tag, releaseAsset.Name)
...
func (k Keeper) GetReleaseAsset(ctx sdk.Context, repositoryId uint64, tag string, name string) (val types.ReleaseAsset, found bool) {
	store := prefix.NewStore(ctx.KVStore(k.storeKey), types.KeyPrefix(types.ReleaseAssetKey))
	releaseAssetKey := fmt.Sprintf("%d-%s-%s", repositoryId, tag, name)
...
func (k Keeper) GetReleaseAssets(ctx sdk.Context, repositoryId uint64, tag string) (list []types.ReleaseAsset) {
	store := prefix.NewStore(ctx.KVStore(k.storeKey), types.KeyPrefix(types.ReleaseAssetKey))
	iterator := sdk.KVStorePrefixIterator(store, []byte(fmt.Sprintf("%d-%s", repositoryId, tag)))
...
func (k Keeper) GetReleaseAssetById(ctx sdk.Context, releaseAssetId uint64) (val types.ReleaseAsset, found bool) {
	...
	releaseAssetKey := fmt.Sprintf("%d-%s-%s", releaseAssetRepositoryMapping.RepositoryId, releaseAssetRepositoryMapping.Tag, releaseAssetRepositoryMapping.Name)
	b := store.Get([]byte(releaseAssetKey))
	if b == nil {
		return val, false
	}
...
func (k Keeper) RemoveReleaseAsset(ctx sdk.Context, repositoryId uint64, tag string, name string) {
	...
	releaseAsset, found := k.GetReleaseAsset(ctx, repositoryId, tag, name)
	if !found {
		return
	}
	...
	k.RemoveReleaseAssetRepositoryMapping(ctx, releaseAsset.Id)
}
...
func (k Keeper) RemoveReleaseAssetRepositoryMapping(ctx sdk.Context, releaseAssetId uint64) {
	store := prefix.NewStore(ctx.KVStore(k.storeKey), types.KeyPrefix(types.ReleaseAssetRepositoryMappingKey))
	store.Delete(GetReleaseAssetIdBytes(releaseAssetId))
}
