...
func SimulateMsgDeleteBranch(
	...
	return func(r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context, accs []simtypes.Account, chainID string,
		...
		var (
			...
			branch     = types.Branch{}
			msg        = &types.MsgDeleteBranch{}
			allBranch  = k.GetAllBranch(ctx)
			found      = false
		)
		for _, obj := range allBranch {
		...
		if !found {
			return simtypes.NoOpMsg(types.ModuleName, msg.Type(), "branch creator not found"), nil, nil
		}
