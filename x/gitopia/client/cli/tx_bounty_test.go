...
func TestCreateBounty(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdCreateBounty(), args)
			if tc.err != nil {
...
func TestUpdateBounty(t *testing.T) {
	...
	_, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdCreateBounty(), args)
	require.NoError(t, err)
...
func TestDeleteBounty(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		tc := tc
		t.Run(tc.desc, func(t *testing.T) {
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdDeleteBounty(), append([]string{tc.id}, tc.args...))
			if tc.err != nil {
				require.ErrorIs(t, err, tc.err)
			} else {
				require.NoError(t, err)
