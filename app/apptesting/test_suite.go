...
func (s *KeeperTestHelper) Setup() {
	...
	s.Ctx = s.App.BaseApp.NewContext(false, tmtypes.Header{Height: 1, ChainID: "gitopia-1", Time: time.Now().UTC()})
...
func TestMessageAuthzSerialization(t *testing.T, msg sdk.Msg) {
	...
	expiration := someDate.Add(time.Hour)
	const (
		mockGranter string = "gitopia1abc"
		mockGrantee string = "gitopia1xyz"
	)
	...
	grant, err := authz.NewGrant(someDate, authz.NewGenericAuthorization(typeURL), &expiration)
