...
var (
	encodingConfig gitopiaparams.EncodingConfig
	cdc            codec.Codec
	txConfig       client.TxConfig
)

func init() {
	encodingConfig = gitopiaparams.MakeEncodingConfig()
...
func newChain() (*chain, error) {
	tmpDir, err := os.MkdirTemp("", "gitopia-e2e-testnet-")
	if err != nil {
		return nil, err
	}
...
func (c *chain) createValidator(index int) *validator {
	return &validator{
		chain:   c,
		index:   index,
		moniker: fmt.Sprintf("%s-gitopia-%d", c.id, index),
	}
}
