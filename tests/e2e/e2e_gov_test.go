...
2. Submission, deposit and vote of proposal to spend from the community pool to send lores to a recipient
...
func (s *IntegrationTestSuite) GovCommunityPoolSpend() {
	...
	sendAmount := sdk.NewCoin(uloreDenom, sdk.NewInt(10000000)) // 10ulore
	s.writeGovCommunitySpendProposal(s.chainA, sendAmount, recipient)

	beforeRecipientBalance, err := getSpecificBalance(chainAAPIEndpoint, recipient, uloreDenom)
	...
	s.Require().Eventually(
		func() bool {
			afterRecipientBalance, err := getSpecificBalance(chainAAPIEndpoint, recipient, uloreDenom)
			s.Require().NoError(err)
