...
func (s *IntegrationTestSuite) testBankTokenTransfer() {
	s.Run("send_tokens_between_accounts", func() {
		...
		s.Require().Eventually(
			func() bool {
				beforeAliceUAtomBalance, err = getSpecificBalance(chainEndpoint, alice.String(), uloreDenom)
				s.Require().NoError(err)

				beforeBobUAtomBalance, err = getSpecificBalance(chainEndpoint, bob.String(), uloreDenom)
				s.Require().NoError(err)

				beforeCharlieUAtomBalance, err = getSpecificBalance(chainEndpoint, charlie.String(), uloreDenom)
				s.Require().NoError(err)
		...
		s.Require().Eventually(
			func() bool {
				afterAliceUAtomBalance, err = getSpecificBalance(chainEndpoint, alice.String(), uloreDenom)
				s.Require().NoError(err)

				afterBobUAtomBalance, err = getSpecificBalance(chainEndpoint, bob.String(), uloreDenom)
				s.Require().NoError(err)
		...
		s.Require().Eventually(
			func() bool {
				...
				afterCharlieUAtomBalance, err = getSpecificBalance(chainEndpoint, charlie.String(), uloreDenom)
				s.Require().NoError(err)
