...
func (s *IntegrationTestSuite) sendIBC(c *chain, valIdx int, sender, recipient, token, fees, note string, expErr bool) {
	...
	if expErr {
		...
		s.executeGitopiaTxCommand(ctx, c, ibcCmd, valIdx, s.defaultExecValidation(c, valIdx))
...
func (s *IntegrationTestSuite) testIBCTokenTransfer() {
	s.Run("send_ulore_to_chainB", func() {
...
func (s *IntegrationTestSuite) testMultihopIBCTokenTransfer() {
	...
	s.Run("send_successful_multihop_ulore_to_chainA_from_chainA", func() {
		// require the recipient account receives the IBC tokens (IBC packets ACKd)
		var (
			err error
		)
		...
		s.Require().Eventually(
			func() bool {
				beforeSenderUAtomBalance, err = getSpecificBalance(chainAAPIEndpoint, sender, uloreDenom)
				s.Require().NoError(err)

				beforeRecipientUAtomBalance, err = getSpecificBalance(chainAAPIEndpoint, recipient, uloreDenom)
				s.Require().NoError(err)
		...
		s.Require().Eventually(
			func() bool {
				...
				afterRecipientUAtomBalance, err := getSpecificBalance(chainAAPIEndpoint, recipient, uloreDenom)
				s.Require().NoError(err)
...
func (s *IntegrationTestSuite) testFailedMultihopIBCTokenTransfer() {
	...
	s.Run("send_failed_multihop_ulore_to_chainA_from_chainA", func() {
		address, _ := s.chainA.validators[0].keyInfo.GetAddress()
		sender := address.String()
