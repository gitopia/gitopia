...
var (
	...
	vestingAmountVested     = sdk.NewCoin(uloreDenom, sdk.NewInt(99900000000))
	vestingAmount           = sdk.NewCoin(uloreDenom, sdk.NewInt(350000))
	vestingBalance          = sdk.NewCoins(vestingAmountVested).Add(vestingAmount)
	vestingDelegationAmount = sdk.NewCoin(uloreDenom, sdk.NewInt(500000000))
	vestingDelegationFees   = sdk.NewCoin(uloreDenom, sdk.NewInt(1))
)
...
func (s *IntegrationTestSuite) testDelayedVestingAccount(api string) {
	...
	s.Run("test delayed vesting genesis account", func() {
		...
		balance, err := getSpecificBalance(api, vestingDelayedAcc.String(), uloreDenom)
		s.Require().NoError(err)
		s.Require().Equal(vestingBalance.AmountOf(uloreDenom), balance.Amount)

		// Delegate coins should succeed
		s.execDelegate(chain, valIdx, vestingDelegationAmount.String(), valOpAddr,
			vestingDelayedAcc.String(), gitopiaHomePath, vestingDelegationFees.String())
...
func (s *IntegrationTestSuite) testContinuousVestingAccount(api string) {
	s.Run("test continuous vesting genesis account", func() {
		...
		//	Check address balance
		balance, err := getSpecificBalance(api, continuousVestingAcc.String(), uloreDenom)
		s.Require().NoError(err)
		s.Require().Equal(vestingBalance.AmountOf(uloreDenom), balance.Amount)

		// Delegate coins should succeed
		s.execDelegate(chain, valIdx, vestingDelegationAmount.String(),
			valOpAddr, continuousVestingAcc.String(), gitopiaHomePath, vestingDelegationFees.String())
...
func (s *IntegrationTestSuite) testPeriodicVestingAccount(api string) { //nolint:unused
	...
	s.Run("test periodic vesting genesis account", func() {
		...
		s.execCreatePeriodicVestingAccount(
			chain,
			periodicVestingAddr,
			filepath.Join(gitopiaHomePath, vestingPeriodFile),
		...
		//	Check address balance
		balance, err := getSpecificBalance(api, periodicVestingAddr, uloreDenom)
		s.Require().NoError(err)

		expectedBalance := sdk.NewCoin(uloreDenom, sdk.NewInt(0))
		for _, period := range acc.VestingPeriods {
			// _, coin := ante.Find(period.Amount, uloreDenom)
			_, coin := period.Amount.Find(uloreDenom)
		...
		// Delegate coins should succeed
		s.execDelegate(chain, valIdx, vestingDelegationAmount.String(), valOpAddr,
			periodicVestingAddr, gitopiaHomePath, vestingDelegationFees.String())
...
func generateVestingPeriod() ([]byte, error) {
	p := vestingPeriod{
		...
		Periods: []period{
			{
				Coins:  "850000000" + uloreDenom,
				Length: 35,
			},
			{
				Coins:  "2000000000" + uloreDenom,
				Length: 35,
			},
		},
	}
