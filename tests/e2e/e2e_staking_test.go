...
func (s *IntegrationTestSuite) testStaking() {
	...
	fees := sdk.NewCoin(uloreDenom, sdk.NewInt(1))
	...
	delegation := sdk.NewCoin(uloreDenom, delegationAmount) // 500 lore

	// Alice delegate ulore to Validator A
	s.execDelegate(s.chainA, 0, delegation.String(), validatorAddressA, delegatorAddress.String(), gitopiaHomePath, fees.String())
	...
	redelegation := sdk.NewCoin(uloreDenom, redelegationAmount) // 250 lore

	// Alice re-delegate half of her ulore delegation from Validator A to Validator B
	s.execRedelegate(s.chainA, 0, redelegation.String(), validatorAddressA, validatorAddressB, delegatorAddress.String(), gitopiaHomePath, fees.String())
	...
	s.Require().Eventually(
		func() bool {
			...
			currDelegation = sdk.NewCoin(uloreDenom, currDelegationAmount)
	...
	// Alice unbonds all her ulore delegation from Validator A
	s.execUnbondDelegation(s.chainA, 0, currDelegation.String(), validatorAddressA, delegatorAddress.String(), gitopiaHomePath, fees.String())
